...
COLOR_RAMP = [
    rgb_to_color([74/255, 221/255, 179/255]),
    rgb_to_color([109/255, 183/255, 176/255]),
    rgb_to_color([161/255, 126/255, 172/255]),
    rgb_to_color([205/255, 79/255, 168/255]),
    rgb_to_color([249/255, 32/255, 165/255])
]


def PDF_bivariate_normal(x_1, x_2, mu_1=0, mu_2=0, sigma_1=1, sigma_2=1, rho=0):

    normalizing_const = 1/(2 * math.pi * sigma_1 *
                           sigma_2 * math.sqrt(1 - rho**2))
...
class AdjustMu(ThreeDScene):
    ...
    def construct(self):
        ...
        mu_1_text_group = VGroup(
            mu_1_tex, mu_1_value_text).arrange(RIGHT, buff=0.5)
        ...
        mu_2_text_group = VGroup(
            mu_2_tex, mu_2_value_text).arrange(RIGHT, buff=0.5)

        text = VGroup(mu_1_text_group, mu_2_text_group).arrange(
            RIGHT, buff=2).move_to(UP*3.5)
...
class AdjustSigma(ThreeDScene):
    ...
    def construct(self):
        ...
        sigma_1_text_group = VGroup(
            sigma_1_tex, sigma_1_value_text).arrange(RIGHT, buff=0.5)
        ...
        sigma_2_text_group = VGroup(
            sigma_2_tex, sigma_2_value_text).arrange(RIGHT, buff=0.5)

        text = VGroup(sigma_1_text_group, sigma_2_text_group).arrange(
            RIGHT, buff=2).move_to(UP*3.5)
        # Fix position of text so 3D camera movements do not impact position
        self.add_fixed_in_frame_mobjects(text)
...
class AdjustRho(ThreeDScene):
    ...
    def construct(self):
        ...
        text = VGroup(rho_tex, rho_value_text).arrange(
            RIGHT, buff=0.5).move_to(UP*3.5)
        # Fix position of text so 3D camera movements do not impact position
        self.add_fixed_in_frame_mobjects(text)
